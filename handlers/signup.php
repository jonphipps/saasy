...
if ($customer) {
	$this->redirect(
		$this->is_https() ? 'https://' . $www . saasy\App::base_domain() . '/user/signup' :
...
xdebug_break();
			...
			$form->controller->redirect(
				$form->controller->is_https() ? 'https://' . $customer->domain() . '/' :
